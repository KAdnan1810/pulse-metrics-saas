    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-12 transition-colors">
      ...
      <main className="max-w-6xl mx-auto px-6 mt-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">API Usage & Rate Limiting</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">Real-time quota monitoring backed by Redis & PostgreSQL</p>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-transparent p-2.5 rounded-lg transition"
        ...
        {pingStatus && (
          <div className={`p-4 rounded-xl mb-6 flex items-center justify-between border transition-all duration-300 ${
            ...
            <span className="text-xs opacity-60">auto-closes in 4s</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm transition-colors">
            ...
            <p className="text-3xl font-bold text-slate-900 dark:text-white">{metrics?.planName}</p>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm transition-colors">
            ...
            <p className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{metrics?.quotaUsagePercentage}%</p>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm transition-colors">
          <div className="p-6 border-b border-slate-200 dark:border-slate-800">
            <h3 className="font-semibold text-slate-900 dark:text-slate-200">Recent API Audit Logs</h3>
          </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                {metrics?.recentLogs?.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="px-6 py-8 text-center text-slate-400">No requests logged yet. Click 'Test API Request' above.</td>
                  </tr>
                ) : (
                  metrics?.recentLogs?.map((log, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                      <td className="px-6 py-3 font-mono text-xs text-indigo-600 dark:text-indigo-300">{log.endpoint}</td>
                      ...
                      <td className="px-6 py-3 text-slate-500 dark:text-slate-400">{log.responseTimeMs}ms</td>
                      <td className="px-6 py-3 text-slate-400 dark:text-slate-500 text-xs">{new Date(log.timestamp).toLocaleString()}</td>
                    </tr>
                  ))
                )}
              </tbody>
